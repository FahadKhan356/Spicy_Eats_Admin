...
Widget buildStats({required WidgetRef ref}) {
  ...
  builder: (context, snapshot) {
   ...
   final avgPrice=ref.read(menuManagerController).totalAvgPrice(snapshot: dishes!);
...
final itemsUnavailableLength= itemsTotalLength-itemsAvailableLength;
     ...
     return IntrinsicHeight(
       child: Row(
         ...
         children: [
            buildStatCard('Total Items', itemsTotalLength.toString(), MyAppColor.orangePrimary,context),
             const SizedBox(width: 16),
           buildStatCard('Available', itemsAvailableLength.toString(), MyAppColor.orangePrimary,context),
             const SizedBox(width: 16),
          buildStatCard('Unavailable', itemsUnavailableLength.toString(), MyAppColor.orangePrimary,context),
          const SizedBox(width: 16),
          buildStatCard('Avg Price', '\$${avgPrice.toStringAsFixed(2)}',  MyAppColor.orangePrimary,context),
         ],
       ),
     );
  },
